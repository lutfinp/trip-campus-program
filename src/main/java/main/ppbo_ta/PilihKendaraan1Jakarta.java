...
public class PilihKendaraan1Jakarta implements Initializable {
    @FXML
    ...
    private Stage stage;
    private Scene scene;
    public void switchToKendaraan2(ActionEvent event) throws IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("pilih-kendaraan2.fxml"));
        root = loader.load();

        user.setListKendaraan(myKendaraan);

        PilihKendaraan2 pilihKendaraan2 = loader.getController();
        pilihKendaraan2.setInfo(user);
        pilihKendaraan2.setTopLabels();

        stage = (Stage)((Node)event.getSource()).getScene().getWindow();
        scene = new Scene(root);
        String css = this.getClass().getResource("pilih-kendaraan2.css").toExternalForm();
